...
export class DatabaseModule {
    ...
    static forFeature(
        models?: { model: Type<any>, discriminators?: Type<any>[], }[],
        connectionName?: string
    ): DynamicModule {
        if (!models) return MongooseModule.forFeature([], connectionName);

        const models_: ModelDefinition[] = models.map(model => {
            return this.generateModelDefinition(model.model, model.discriminators || [])
        })
        return MongooseModule.forFeature(models_, connectionName);
    }
